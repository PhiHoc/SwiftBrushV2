...
def tokenize_prompt(prompt, tokenizer):
    inputs = tokenizer(prompt, max_length=tokenizer.model_max_length, padding="max_length", truncation=True,
                       return_tensors="pt")
    return inputs.input_ids


def load_random_image_from_dataset(train_data_dir, resolution):
    # Load a random image from the train data directory
    image_files = [f for f in os.listdir(train_data_dir) if f.endswith(('.png', '.jpg', '.jpeg'))]
    random_image_path = os.path.join(train_data_dir, random.choice(image_files))

    # Open image, resize to match the model's resolution, and normalize
    image = Image.open(random_image_path).convert("RGB")
    image = image.resize((resolution, resolution))
    image = np.array(image).astype(np.float32) / 255.0  # Normalize to [0, 1]
    image = torch.tensor(image).permute(2, 0, 1).unsqueeze(0)  # Change shape to (1, C, H, W)

    return image


def generate_image(prompt, tokenizer, text_encoder, unet, vae, scheduler, device, train_data_dir, resolution):
    input_ids = tokenize_prompt([prompt], tokenizer).to(device)
    encoder_hidden_state = text_encoder(input_ids)[0]

    # Load a random image from the dataset to use as latents
    target_image = load_random_image_from_dataset(train_data_dir, resolution).to(device)

    # Encode the target image into latents
    latents = vae.encode(target_image).latent_dist.sample() * vae.config.scaling_factor
    ...
    with torch.no_grad():
        model_pred = unet(latents, timestep, encoder_hidden_state).sample
        if model_pred.shape[1] == latents.shape[1] * 2:
            model_pred, _ = torch.split(model_pred, latents.shape[1], dim=1)

        pred_original_sample = (latents - sigma_t * model_pred) / alpha_t
...
def sample_func(args, in_queue, gpu_id, process_id, environments):
    ...
    while True:
        ...
        for prompt, save_path in zip(prompts, save_paths):
            image = generate_image(prompt, tokenizer, text_encoder, unet, vae, scheduler, device, args.train_data_dir,
                                   args.resolution)
