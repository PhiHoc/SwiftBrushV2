...
class HugFewShotDataset(Dataset):
    ...
    # --- THAY ĐỔI 1: Sửa đổi phương thức parse_syn_data_pd ---
    def parse_syn_data_pd(self, synthetic_dir, filter=True) -> None:
        ...
        for _dir in synthetic_dir:
            ...
            if os.path.exists(meta_dir):
                file_found = True
                print(f"Info: Found and loading '{meta_dir}'")
                ...
                print(f"Warning: '{meta_dir}' not found. Skipping synthetic data for this directory.")

        if file_found:
            self.meta_df = pd.concat(meta_df_list).reset_index(drop=True)
            self.syn_nums = len(self.meta_df)
            print(f"Total synthetic numbers: {self.syn_nums}\n")
        else:
            print("Info: No meta.csv found. Proceeding with real images only.")
