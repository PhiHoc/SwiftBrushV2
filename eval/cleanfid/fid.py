...
def get_folder_features(
    ...
    if ".zip" in fdir:
        ...
        files = sorted(file for ext in EXTENSIONS for file in glob(os.path.join(fdir, f"**/*.{ext}"), recursive=True))
    if verbose:
    ...
    if num is not None:
        ...
        print(files[:10])
    if verbose:
        print(f"Clip to {len(files)} images in the folder {fdir}")

    if custom_image_tranform is not None:
        resize_size = inspect.signature(custom_image_tranform).parameters["resize_size"].default
    else:
        resize_size = "none"
    CACHE_DIR = Path("~/.cache/cleanfid").expanduser()
    FNAME = Path(fdir).resolve().as_posix().replace("/", "_")
    FNAME = f"{FNAME}_{resize_size}_{num}.npz"
    FPATH = CACHE_DIR / FNAME
    FPATH.parent.mkdir(exist_ok=True, parents=True)
    if FPATH.exists():
        np_feats = np.load(FPATH)["feats"]
        print(f"Load feature of shape: {np_feats.shape} from {FPATH}")
    else:
        ...
        np.savez_compressed(FPATH, feats=np_feats)
    return np_feats
