# This file is heavily inspired by the original recall.py script
# from the consistency-model repository.
import argparse
import io
import os
import random
...
def main():
    ...
    evaluator = Evaluator(tf.Session(config=config), batch_size=256)
    ...
    ref_name = Path(args.ref_batch).name
    op = Path("./.cache/recalltf") / f"{ref_name}.npz"
    print("Cache path:", op)
    ...
    if not op.exists():
        ...
        np.savez(
            op.as_posix(),
            ref_stats=ref_stats,
            ref_stats_spatial=ref_stats_spatial,
            ref_acts_0=ref_acts[0],
            ref_acts_1=ref_acts[1],
        )
    else:
        featz = np.load(op, allow_pickle=True)
        ref_acts_0, ref_acts_1, ref_stats, ref_stats_spatial = (
            featz["ref_acts_0"],
            featz["ref_acts_1"],
            featz["ref_stats"],
            featz["ref_stats_spatial"],
        )
        ref_acts = tuple([ref_acts_0, ref_acts_1])
        ref_stats = ref_stats.item()
        ref_stats_spatial = ref_stats_spatial.item()
    ...
    print("Recall:", recall)
    with open("metrics/recall.txt", "a") as f:
        f.write(f"{args.ref_batch}\t{args.sample_batch}\t|Precision:{prec}|Recall:{recall}\n")
...
class Evaluator:
    ...
    def read_activations(self, npz_path: str) -> Tuple[np.ndarray, np.ndarray]:
        if os.path.isdir(npz_path):
            images = list(Path(npz_path).rglob("*.png"))
            ...
            def read_batches(images, bs):
                for file_batch in batch_iterator(images, bs):
                    images = np.array([cv2.imread(str(file), cv2.IMREAD_COLOR) for file in file_batch])
                    yield np.array(images)  # Convert the list of images to a numpy array

            return self.compute_activations(read_batches(images, self.batch_size))
        else:
...
def _read_bytes(fp, size, error_template="ran out of data"):
    """Copied from:
    https://github.com/numpy/numpy/blob/fb215c76967739268de71aa4bda55dd1b062bc2e/numpy/lib/format.py#L788-L886.

    Read from file-like object until size bytes are read. Raises ValueError if not EOF is encountered before size bytes
    are read. Non-blocking objects only supported if they derive from io objects. Required as e.g. ZipExtFile in python
    2.6 can return less data than requested.
    """
    data = bytes()
    while True:
